...
{
    public class WPengtaiChatAuthorised : BaseController
    {
        ...
        [ActionName("WeChatAuthorised")]
        [HttpGet, HttpPost]
        public string WeChatAuthorised(string returnurl, string scope, string isReturnData)
        {
            //判断是否包含授权成功后跳转的地址
            if (string.IsNullOrEmpty(returnurl))
            {
                return CommonMessage.Error(CodeMessageEnum.MissingParameters).ToString();
            }
            else
            {
                //判断最后授权成功后跳转的地址是否有效
                ...
                if (string.IsNullOrEmpty(WChatAppid) || string.IsNullOrEmpty(WChatAppSecret))
                    return CommonMessage.Error(CodeMessageEnum.MissingAppidOrSecret).ToString();
                //授权的请求地址
                ...
                string callBackUrl = HttpUtility.UrlEncode(Request.RequestUri.Scheme.ToString() + "/WChatAuthorised/WXAuthorizedCallBack?isReturnData=" + isReturnData + "&Scope=" + scope.ToLower() + "returnUrl=" + returnurl);

                requestAuthorizeUrl = string.Format(requestAuthorizeUrl, WChatAppid, callBackUrl, scope);

                return HttpHelper.GetWebContent(requestAuthorizeUrl);
            }
        }

        [ActionName("WXAuthorizedCallBack")]
        [HttpGet]
        public string WXAuthorizedCallBack()
        {
